...
M = len(intl)
lvol = np.log(vol)
total = np.log(1 + intl) - infl[-M:] # computation of total returns
...
def classicSim(initVol, T):
    # generate Gaussian noise for returns
    noiseSim = np.random.multivariate_normal(np.zeros(2), covMatrix, (T, NSIMS))
    noiseVol = noiseSim[:, :, 0]
    noiseIntl = noiseSim[:, :, 1]
...
def bayesAllSim(initVol, T):
    ...
    # simulate noise with varying Bayesian volatility
    noiseSim = np.random.multivariate_normal(np.zeros(2), corrMatrix, (T, NSIMS))
    noiseVol = noiseSim[:, :, 0] * simStdVol[None, :]
    noiseIntl = noiseSim[:, :, 1] * simStdIntl[None, :]
...
for model in [model0, model1, model2]:
    ...
    for withdrawal in [0.03, 0.04, 0.05]:
        for t in range(T):
            wealth[t+1] = wealth[t] * np.exp(model[t]) - withdrawal * np.ones(NSIMS)
        print('ruin probability with withdrawal rate ', withdrawal)
